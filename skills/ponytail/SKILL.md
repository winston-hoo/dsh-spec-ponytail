...
> adapted for DeepSeek Harness. The condensed form of this ruleset is injected as a
> resident system-prompt section on every request; this file is the full version.
...
## Switching level (DeepSeek Harness)

Send one short message, standalone — the plugin parses it before the model
sees it, so it always takes effect:

| Message | Effect |
|---------|--------|
| `ponytail lite` / `ponytail full` / `ponytail ultra` | Set the level |
| `ponytail` | Report the current level (re-enables at the default level if it was off) |
...
Ponytail governs what you build, not how you talk (pair with a terse-prose
rule for the prose half). "stop ponytail" / "normal mode": revert. Level
persists until changed or process end.
