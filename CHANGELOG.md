...
**给规则集加两条"交付自检"护栏**（钉死一次真实事故的教训：消息列表只交裸骨架、未读角标看不到且刷新即消失）。

背景：用户反馈"不给完整需求文档时，模型按最偷懒原则实现 —— 未读消息数不显示，进入列表不做已读/未读区分，只留一个全部已读"。核对真实会话后确认，这类事故的共同根因是 **「代码路径存在」被当成「功能完成」**，而用户验收的是屏幕上的像素：

- 未读角标"只显示一次就没了"：`loadUnreadCount()` 的 `.catch(() => {})` 在失败/未登录时把 `unreadCount` 写成 **0**，而下拉刷新只走 `loadUserData()`、**根本不重拉未读数** —— 偶发失败被变成永久消失。
- 已读/未读"根本没渲染出来"：红点用 `.msg.unread::before { ... flex:none }`，但 `.msg` 不是 flex 容器，伪元素退化成块级盒子，渲染成**整行色带**，红点从未显示过；文字区分缺失，只剩 `#fff` vs `#fffdf8` 的**肉眼不可辨**色差。
- 功能维度层面：需求说"用户消息列表"，交付只做裸列表 + 单个「全部已读」，已读/未读态、计数、空态都不在。

0.3.2 的两条护栏只管**单个动作**（可选字段被丢弃 / 危险写操作），兜不住**功能维度被整体略过**与**状态不可感知**。本次补两条：

- `lib/ruleset.js` → `## Never lazy about` 追加压缩版（常驻段）：
  1. **看得见 = 真的渲染出来**：要给人看的计数/角标/已读未读标记，必须验渲染结果（`verify the pixels, not the branch`），不能只验代码路径。
  2. **点名的列表 ≠ 裸骨架**：已读/未读态、计数、空态与错误态默认包含；跨刷新存活的状态属于功能本身，**接口失败不得回写为 0**。
- `skills/ponytail/SKILL.md` → `## When NOT to be lazy` 追加同口径完整版，含上述两个事故案例。

配套改动：

- 常驻段体量上限 **2200 → 2700 字符**（新增约 410 字符，留余量避免下次改动即贴顶）：同步 `tests/ruleset.test.js` 与 `scripts/token-audit.js` 的 `LIMIT`。
- `tests/ruleset.test.js` 新增 1 条断言组（三个关键判据在三个档位都必须出现）。
- README 成本表按实测更新（full 2608 / ultra 2604 字符；切档注入 2664），修正"85 条"为 89 条，FAQ 增加本类事故的问答。

验证：单测 **89 条全过**；`token-audit` 退出码 0，常驻段 2607–2608 字符（≈652 token/step）。
...
- **作用域：全机一份、跨工作区共享。** 路径取 `process.cwd()`；**实测那是启动 dsh 的目录，
  不是会话工作区** ——
...
重启后发 `ponytail full`，启动目录下的 `.dsh-ponytail/mode` 当场变成 `full`
