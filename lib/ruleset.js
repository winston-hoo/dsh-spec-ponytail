...
export function renderRuleset(mode) {
  ...
  return [
    ...
    '',
    'What the user can see is not done because the code path exists. A count, badge, or read/unread mark must be',
    'perceivable on the rendered screen — verify the pixels, not the branch. A named list is not a request for its',
    'bare skeleton: read state, counts, empty and error states belong to it unless told otherwise. State that must',
    'outlive a refresh is part of the feature — never write a failed fetch back as zero.',
    '',
    '## Output',
