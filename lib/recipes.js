...
//
// 解决的问题：同类需求每次都要从头推导改法。结合 dsh-spec-forge 的"召回 → 干活 → 沉淀"
// 思路，但按用户选型**砍到最瘦**——模板只保留「触发 + 做法 + 禁区」三要素（做法清单），
// 不要澄清清单、不要提示词模板。它服务的是"下次同类需求快速照做"，不是需求分层。
//
// 与 dsh-spec-forge 模板库的区别：spec-forge 管"把模糊需求锻造成规格"（含 classify/
// triage/distill 全套）；这里只做一件事——**命中既有做法清单就注入，省掉重新推导**。
// 两个插件各自独立目录，不强耦合：这里绝不读 spec-forge 的内部格式。
//
...
// ---------- 匹配（轻量 CJK 捕获，不拉 spec-forge 的整包 fingerprint） ----------
